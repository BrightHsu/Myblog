...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        ...
        if request.args.get('reply'):
            comment = Comment.query.get_or_404(request.args.get('reply'))

            reply = Reply(author=author, email=email, site=site, body=body, reviewed=reviewed, from_admin=from_admin,
                          comment=comment)
            db.session.add(reply)
        else:
            comment = Comment(author=author, email=email, site=site, body=body, reviewed=reviewed, from_admin=from_admin,
                              post=post)
        ...
        if current_user.is_authenticated:
            ...
            flash('感谢您的评论, 该评论需要通过审核后才能发送到全世界！', 'info')
...
def reply_comment(comment_id):
    ...
    return redirect(url_for('.show_post', post_id=comment.post_id, reply=comment_id, author=comment.author, r_id=r_id,admin=comment.from_admin)+'#comment-form')